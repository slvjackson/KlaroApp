...
async function checkMetroHealth() {
  try {
    const response = await fetch("workspaceapi-server-production-a4f2.up.railway.app/status", {
...
async function downloadBundle(platform, timestamp) {
  ...
  const url = new URL(`workspaceapi-server-production-a4f2.up.railway.app/${bundlePath}.bundle`);
...
async function downloadManifest(platform) {
  ...
  try {
    ...
    const response = await fetch("workspaceapi-server-production-a4f2.up.railway.app/manifest", {
...
function extractAssets(timestamp) {
  ...
  const extractFromBundle = (bundle, platform) => {
    for (const match of bundle.matchAll(assetPattern)) {
      ...
      const tempUrl = new URL(`workspaceapi-server-production-a4f2.up.railway.app${originalPath}`);
...
async function downloadAssets(assets, timestamp) {
  ...
  const downloadPromises = assets.map(async (asset) => {
    const tempUrl = new URL(`workspaceapi-server-production-a4f2.up.railway.app${asset.originalPath}`);
    const unstablePath = tempUrl.searchParams.get("unstable_path");
...
function updateBundleUrls(timestamp, baseUrl) {
  const updateForPlatform = (platform) => {
    ...
    bundle = bundle.replace(
      ...
      (_match, capturedPath) => {
        const tempUrl = new URL(`workspaceapi-server-production-a4f2.up.railway.app${capturedPath}`);
        const unstablePath = tempUrl.searchParams.get("unstable_path");
